...
public class StudentNewapplicationServlet extends HttpServlet {
		 ...
		 HttpSession session = request.getSession();
				 String email = request.getParameter("email");
		   		...
		   		deptid=2;
					...
					if(d.equals(" ")){
						session.setAttribute("errmsg", "* please select a option");
					...
					if(d.equals("CS")){
							...
							if(m1!=null){
								...
								url="/Student_departmant_questions.jsp";
								getServletContext().getRequestDispatcher(url).forward(request, response);
